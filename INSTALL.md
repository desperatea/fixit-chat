...
FixIT Chat и GLPI — два отдельных сервера. Они **не общаются** напрямую. Связь только через браузер пользователя + общий секрет для подписи токенов:

```
[helpdesk.fixitmail.ru]              [chat.fixitmail.ru]
   GLPI + плагин fixitchat             FixIT Chat (Docker)
         │                                    │
         └── Плагин генерирует                └── Backend проверяет
             подписанный токен                    HMAC подпись
                    │                                    │
                    └────── Браузер пользователя ─────────┘
```

> **Важно:** GLPI контейнеры (`glpi`, `glpi-db`) в docker-compose.yml нужны **только для dev-тестирования**.
> На проде GLPI уже установлена — разворачивать свою не нужно.
> Запускать FixIT Chat на проде без GLPI контейнеров:
> ```bash
> docker compose up -d postgres redis backend admin widget nginx
> ```

### Что нужно

- GLPI 10.0+ установленная и работающая (например `helpdesk.fixitmail.ru`)
- Доступ к файловой системе сервера GLPI (для установки плагина)
- Доступ к конфигурации веб-сервера (Apache или nginx)
- Работающий FixIT Chat на отдельном сервере (например `chat.fixitmail.ru`)
...
scp -r glpi/plugins/fixitchat user@glpi-server:/var/www/html/glpi/plugins/
```

На сервере GLPI должна появиться структура:
```
/var/www/html/glpi/plugins/fixitchat/
├── setup.php                  # Хуки, генерация токена
└── js/
    ├── widget_inject.js       # Для авторизованных страниц
    └── widget_inject_anon.js  # Для страницы логина
```

> **Файл `glpi/fixit_widget.php`** в репозитории — старый standalone скрипт. На прод он **НЕ нужен**.
...
### Шаг 9: Настроить URL GLPI для создания заявок

В файле `admin/src/pages/ChatPage.tsx` найти строку:
```typescript
const glpiBase = 'http://localhost:8090'; // TODO: move to settings
```

Заменить на URL рабочей GLPI:
```typescript
const glpiBase = 'https://helpdesk.fixitmail.ru';
```
...
Это нужно для кнопки «Создать заявку» в чате — она открывает GLPI с предзаполненными данными (организация, заявитель, описание из чата).

### Что видит агент в админке

В сессии, созданной через GLPI, агент видит:
- **Имя** — из профиля GLPI
- **Телефон** — из профиля GLPI (если заполнен), можно отредактировать кликом
- **Организация** — Entity пользователя GLPI
- **Кнопка «Создать заявку»** — открывает GLPI с предзаполненными организацией, заявителем и описанием из чата
- **custom_fields.glpi_user_id** — ID пользователя в GLPI
- **custom_fields.glpi_entity_id** — ID организации в GLPI

### Персистентная сессия

Один GLPI-пользователь = одна открытая сессия чата. Если пользователь зайдёт с другого ПК — откроется та же сессия. После закрытия сессии — следующее обращение создаст новую.
