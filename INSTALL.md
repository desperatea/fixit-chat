# FixIT Chat — Инструкция по запуску (тестовый режим)
...
Если сервер на машине `10.66.1.15`, то коллеги из той же сети могут открыть:

| Что | Адрес |
|-----|-------|
| Тестовая страница с виджетом | http://10.66.1.15/test |
| Админ-панель | http://10.66.1.15/admin/ |
...
1. Откройте **http://10.66.1.15/test** — это имитация сайта клиента с виджетом чата
2. Нажмите на кнопку чата (правый нижний угол)
3. Заполните форму, отправьте сообщение
4. Откройте **http://10.66.1.15/admin/** на другом компьютере/вкладке
